...
- **Testing the catch with an automated/computer-use harness is hard**: monkeys are small and flee, and the maps are large (~120²). A human catches them easily by cornering; an automated tester struggles to sustain the chase. For a reliable automated catch demo you can *temporarily* lower `MONKEY_WALK_SPEED`/`MONKEY_SPRINT_SPEED` and/or raise `CATCH_RANGE` in `constants.js`, then revert before committing. Do not ship those test values. Two non-obvious gotchas: (1) the red alarm beacon floats ~2.2 m **above** the monkey and now scales up with distance, so aim at the monkey's feet on the ground, not the beacon; and (2) the catch also requires facing the target (`PLAYER.CATCH_FOV_DOT`). For a fully hands-off "catch in place" demo, temporarily set `CATCH_FOV_DOT: -1` and a huge `CATCH_RANGE` (e.g. 200) so every left-click while pointer-locked catches the nearest monkey with no navigation — then revert all of it.
...
- **MonkeyBreak is modular**: the shell `client/maps/MonkeyBreakMap.js` composes section modules from `client/maps/monkeybreak/*.js` (fixed call order) against the frozen build-context in `monkeybreak/shared.js` — edit the relevant section file, not the shell. Spawns/exits/items are assembled at build time (loops + seeded rng), so `server/mapColliders.js` MONKEY_BREAK data must be re-extracted by building the map headlessly, not read from literals.
- **Prison Escape is a SOLO-only mode** (`MODES.ESCAPE.solo`, `fixedMapId: 'MONKEY_BREAK'` in `client/core/constants.js`): the main-menu map card is locked to MonkeyBreak and `sanitizeModeId` in `server/Room.js` rejects it online.
